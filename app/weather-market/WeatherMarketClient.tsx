...
const marketPrices = [
    { crop: "Tomato", currentPrice: 25, previousPrice: 30, change: -16.7, wholesale: 22, retail: 28, trend: "down" },
    { crop: "Onion", currentPrice: 35, previousPrice: 28, change: 25, wholesale: 32, retail: 38, trend: "up" },
    { crop: "Potato", currentPrice: 18, previousPrice: 18, change: 0, wholesale: 16, retail: 20, trend: "stable" },
    { crop: "Wheat", currentPrice: 22, previousPrice: 21, change: 4.8, wholesale: 20, retail: 24, trend: "up" },
    { crop: "Rice", currentPrice: 45, previousPrice: 47, change: -4.3, wholesale: 42, retail: 48, trend: "down" },
    { crop: "Cotton", currentPrice: 85, previousPrice: 82, change: 3.7, wholesale: 82, retail: 88, trend: "up" },
];
...
export default function WeatherMarketPage() {
    ...
    try {
      ...
      const days = Object.keys(groups)
        ...
        .map((dateStr, idx) => {
          ...
          return {
            day: dayLabel, high, low, condition: mostCommonCond,
      ...
      // fallback mock
      setDailyForecast([ { day: t.today, high: 30, low: 22, condition: t.partlyCloudy, icon: <Cloud className="h-6 w-6" />, rain: 20 }, { day: t.tomorrow, high: 32, low: 24, condition: t.sunny, icon: <Sun className="h-6 w-6" />, rain: 5 }, { day: t.wednesday, high: 29, low: 21, condition: t.rainy, icon: <CloudRain className="h-6 w-6" />, rain: 80 }, { day: t.thursday, high: 27, low: 20, condition: t.cloudy, icon: <Cloud className="h-6 w-6" />, rain: 40 }, { day: t.friday, high: 31, low: 23, condition: t.sunny, icon: <Sun className="h-6 w-6" />, rain: 10 }, { day: t.saturday, high: 33, low: 25, condition: t.sunny, icon: <Sun className="h-6 w-6" />, rain: 0 }, { day: t.sunday, high: 28, low: 22, condition: t.rainy, icon: <CloudRain className="h-6 w-6" />, rain: 70 }, ]);
    }
  };
  ...
  useEffect(() => {
    const generateDynamicAlerts = () => {
      ...
      if (weather.humidity > 85) {
        newAlerts.push({
          ...
          message: `High humidity (${weather.humidity}%) detected. This may increase the risk of fungal diseases for certain crops.`,
        });
      }

      const significantChangeCrop = marketPrices.find(
        (item) => Math.abs(item.change) > 20
      );
      if (significantChangeCrop) {
        newAlerts.push({
          type: "price",
          title: t.priceAlert,
          message: `Major price shift for ${significantChangeCrop.crop}! The price has moved ${significantChangeCrop.trend} by ${significantChangeCrop.change.toFixed(1)}%.`,
        });
      }
    ...
    // ✅ FIX: Removed `t` and `marketPrices` from the dependency array to prevent the infinite loop.
  }, [weather, language]);
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary" className="bg-green-100 text-green-800">3 crops</Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm">Falling Prices</span>
                    <Badge variant="secondary" className="bg-red-100 text-red-800">2 crops</Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm">Stable Prices</span>
                    <Badge variant="secondary" className="bg-gray-100 text-gray-800">1 crop</Badge>
                  </div>
                </div>
              </CardContent>
            </Card>
