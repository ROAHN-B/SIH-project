...
export default function WeatherMarketPage() {
  ...
  const initialCity = searchParams.get("city")?.trim() || "Pune";
  ...
  const [selectedMarket, setSelectedMarket] = useState(
    () => markets.find((m) => m.label.toLowerCase().includes(initialCity.toLowerCase()))?.value ?? "pune"
  );
  ...
  const fetchWeather = async (cityName: string) => {
    ...
    try {
      ...
      setWeather({
        ...
        visibility: Number(data.visibility ? (data.visibility / 1000).toFixed(1) : 0),
    ...
    try {
      ...
      const days = Object.keys(groups).slice(0, 7).map((dateStr, idx) => {
        ...
        const mostCommonCond = conditions
          ...
          .pop()!;
        const avgRain = Math.round((rainProbs.reduce((a, b) => a + b, 0) / rainProbs.length) * 100);
      ...
      // fallback mock
      setDailyForecast([
        { day: t.today, high: 30, low: 22, condition: t.partlyCloudy, icon: <Cloud className="h-6 w-6" />, rain: 20 },
        { day: t.tomorrow, high: 32, low: 24, condition: t.sunny, icon: <Sun className="h-6 w-6" />, rain: 5 },
        { day: t.wednesday, high: 29, low: 21, condition: t.rainy, icon: <CloudRain className="h-6 w-6" />, rain: 80 },
        { day: t.thursday, high: 27, low: 20, condition: t.cloudy, icon: <Cloud className="h-6 w-6" />, rain: 40 },
        { day: t.friday, high: 31, low: 23, condition: t.sunny, icon: <Sun className="h-6 w-6" />, rain: 10 },
        { day: t.saturday, high: 33, low: 25, condition: t.sunny, icon: <Sun className="h-6 w-6" />, rain: 0 },
        { day: t.sunday, high: 28, low: 22, condition: t.rainy, icon: <CloudRain className="h-6 w-6" />, rain: 70 },
      ]);
    }
  };
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-gray-900">{t.weatherMarketInfo}</h1>
            </div>
          </div>
        </div>
      </header>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            {t.wholesale}: ₹{item.wholesale} | {t.retail}: ₹{item.retail}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
